...
function hideDoctorInfo() {
	...
	document.getElementById("doctor_info_label").style.display = "none";
	document.getElementById("doctor_info").style.display = "none";
	document.getElementById('address_label').innerHTML = "<b>Home Address</b>";
}

function showDoctorInfo() {
	document.getElementById("specialty").style.display = "block";
	document.getElementById("doctor_info_label").style.display = "block";
	document.getElementById("doctor_info").style.display = "block";
	document.getElementById('address_label').innerHTML = "<b>Doctor's Office Address</b>";
}
...
function registerPOST() {
	...
	if(isValidForm() === true){
		...
		//initLatAndLon();  Do i need to implement this?
		...
		xhr.onload = function() {
			if (xhr.readyState === 4 && xhr.status === 200) {      // 3. Ti tha ginei an erthei h sosth apatnhsh
				...
				document.getElementById('passwordErrorMessage2').innerHTML = 'Request was successfulll. Returned status of ' + xhr.status + "<br>";
				
				//const responseData = JSON.parse(xhr.responseText);                                  // this may cause error
				document.getElementById('passwordErrorMessage2').innerHTML += xhr.responseText;
				alert("Request was successfulll. Returned status of" + xhr.status + " " + xhr.responseText);
				
				showLoginRegisterForm();
				
			} else if (xhr.status !== 200) {    // 4. Ti tha ginei an ertei h lathos apanthsh
				document.getElementById('passwordErrorMessage2').innerHTML = 'Request failed. Returned status of ' + xhr.status + "<br>";
		...
		// Check if it is User or Doctor and send two different post requests
		xhr.open('POST', 'RegisterUser'); // 4. Pou kai me ti stelnoume to request
